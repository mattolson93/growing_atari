...
def prepro(img):
    if img.shape[0] > 80: 
        img = img[35:195].astype(np.float32)
        img2 = imresize(img, (80,80))
        
    return img/255. , img2.mean(2).reshape(1,80,80)/255.
...
class MultiEnvironment():
    ...
    def reset(self):
        ...
        for env in self.envs:
            ...
            rgbs.append(rgb)
            bws.append(bw)
        return np.array(rgbs, ndmin = 4), np.array(bws, ndmin = 4)
